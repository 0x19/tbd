...
| `ws_mux` fails but `ws_echo` passes | The socket is up, so this is the call itself: the RPC is not public here (its backend is missing from `[services]`, or the annotation is gone), or the backend is down. | The protocol's startup log (`transcoded routes bound`), `/readyz` for the backend, and the `error` frame's `code`. | Register the backend or restore the annotation. |
