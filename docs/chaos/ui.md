...
| Overview · Environment card | Developers overview resources | Where this environment points: the validate target per kind (collapsed to one line when every kind is behind the same Envoy), what is on disk as tiles with counts that link to the matching page (scenarios, campaigns, findings, run records), and the observability tools as tiles that open Grafana, traces, logs, metrics, profiles and the Envoy admin. A Runbook button opens the knowledge base page for reading a failure. |
