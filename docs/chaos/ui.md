...
|---|---|---|
| Overview | Ecommerce dashboard 1 + Developers overview | KPI strip (stack health, last validate, runs and failure rate over 24 h with deltas against the previous day), throughput of the last scenario run over the previous run of the same scenario, latency p50/p90/p99 per scenario as stacked bars, load runs (throughput split into served and failed, so an error rate is visible without hovering, a p99 line, a red marker under any run with errors, the latest run's numbers as the headline), validate history (last result and checks, pass rate over the last forty, a pass/fail strip linking to each run), the Stack card beside live activity (the last ten minutes, six rows, one row per run once it finishes, consecutive queue changes collapsed, "Show more" for the rest of the session), recent runs. The header and the context strip sit above the KPI strip. |
| Overview · Stack card | Payments dashboard status list | Up, faulted and stopped as chips, requests served and failed since start, a donut of each instance's share of the requests (the services' own counters) and one row per instance: health dot, name, kind, its state (the injected behaviour, the store's behaviour, or "healthy"), address, dependency, failed count and a share bar in the slice's colour. Hovering a row lifts its slice and vice versa. |
| Overview · Header | Developers overview header | The page actions are the kit's: one segmented button group opening this environment's Grafana dashboards, traces, logs, metrics, profiles and the Envoy admin (labels collapse to icons on a narrow screen), then Refresh. Under the title, a context strip: the environment name with a health dot, one live dot per instance (its name, kind and state on hover) with the up and faulted counts, what validate hits (one host and a port per kind, or a single URL when every kind sits behind the same Envoy, the full list on hover), the topology file, counts that link to their page (scenarios, campaigns, findings, runs) and the runbook. |
| Stack | Payments webhooks | Two summary cards (integrity bar per instance, service traffic), an instance list with health dots and Stop / Start / Fault / Replica (the fault dialog picks the surface for a kind with store faults: the service refuses calls, its store fails like a database, losing the acknowledgement of a write that stands), an "added" chip and Remove on instances created at runtime, an "Add instance" dialog whose kind picker and fields come from `overview.kinds` (a dependency field is a picker over running instances of that kind), and a detail sheet per instance with the same actions, key/value rows and the session's activity. |
...
| Runs | Developers events & logs | The sidebar lists "All", one entry per run kind (stress runs under the Stress group) and one "<Kind> runs" per registered service kind (`/runs/?service=<kind>`, from the API's kinds, so a scaffolded service appears without a UI change); only the entry matching the location's query is marked current. The queue panel (what waits for the slot, remove one or clear all), filter rail (kind, status, scenario with counts), search, refresh and Live, table with a totals row and a "scheduled" chip on runs a schedule started. Sidebar sub-items deep-link by kind. |
| Run | Payments delivery simulator + order lifecycle | Lifecycle strip, stat row, the per-second chart, assertions, timeline with OK/ERROR chips, a latency heat grid per operation (darker is slower), error classes, per target and service. Live while running, replayed when opened late. A stress run shows its stages (setup / warmup / run / shrink / teardown), the invariants with evaluations and violations as they arrive over SSE, the findings found so far with links, and for a replay run whether it reproduced. A sweep adds the point it is measuring while it runs, then a chart of latency against the swept value with the p99's confidence band and the knee marked, and the same numbers as a table. |
| Load | Payments delivery simulator | A policy rail (shape, operations grouped by the kind they target, one target per kind: the serve stack, the deployed stack through Envoy from `[targets]`, or a URL; unit suffixes and info tooltips; expected requests and concurrency computed live; why it cannot run yet), a load window stat row, previous load runs with a throughput sparkline, the request body. The same form (`load-shape.tsx`, `lib/load.ts`) builds a schedule's load job. A run's page shows what operations metered beside their latency (counters with a rate, timings as quantiles) under the latency grid. |
...
| Schedules | Ecommerce product list + settings dialog | A Slack card (this environment's channel and outcome filter, or how to turn it on, with "Send a test message"), then the cron jobs: a switch per row, what runs, the preset or raw cron, next fire time, last run chip, the Slack mode, fired and skipped counts, "Run now" and a row menu; a dialog to create or edit (name, what to run with a scenario picker or, for load, the load page's full form: shape, operation mix and a target per kind, so a scheduled ledger load can run against the deployed ledger; a preset or custom cron in UTC, Slack: failures / every outcome / never, enabled). The list describes a load job by its shape, mix and targets. `?new=scenario:<id>` opens it prefilled from a scenario's row menu. |
| Campaigns | Ecommerce product list | The Stress group: every campaign file (`stress/`) with name, description, whether it has a stack of its own, the last run's outcome and findings, "Run" and a row menu (edit, runs, findings, delete). |
...
⌘K, notifications and theme controls, a user menu with initials, name, email, role and
sign-out links when Envoy forwards an identity, the breadcrumb sub-header without the
kit's second search box), the theme preset
