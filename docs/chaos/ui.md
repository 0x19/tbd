...
| Overview · Environment band | Developers overview masthead | The first thing on the page, under the title: the environment name with a health dot, one live dot per instance (hover for its name, kind and state) with the up and faulted counts, what validate hits (one host and a port per kind, or one URL when every kind sits behind the same Envoy), the topology file, counts that link to their page (scenarios, campaigns, findings, runs) and a link into the runbook, then tiles that open Grafana dashboards, traces, logs, metrics, profiles and the Envoy admin. |
