...
|---|---|---|
| Overview | Ecommerce dashboard 1 + Developers overview | KPI strip (stack health, last validate, runs and failure rate over 24 h with deltas against the previous day), throughput of the last scenario run over the previous run of the same scenario, latency p50/p90/p99 per scenario as stacked bars, load runs (throughput bars and p99 line over the last twelve, the latest run's numbers as the headline), validate history (last result and checks, pass rate over the last forty, a pass/fail strip linking to each run), stack list, live activity (the last ten minutes, six rows, one row per run once it finishes, consecutive queue changes collapsed, "Show more" for the rest of the session), where to look (the live serve stack by kind, what validate hits, paths, observability links), recent runs. |
