...
| Run | Payments delivery simulator + order lifecycle | Lifecycle strip, stat row, the per-second chart, assertions, timeline with OK/ERROR chips, a latency heat grid per operation (darker is slower), error classes, per target and service. Live while running, replayed when opened late. A stress run shows its stages (setup / warmup / run / shrink / teardown), the invariants with evaluations and violations as they arrive over SSE, the findings found so far with links, and for a replay run whether it reproduced. |
