...
| `llm_generate` | `LlmService/Generate` of one of four short prompts, streamed to its done chunk, as caller `chaos-load` | every chunk in order, the stub flag constant within the stream, a done chunk with the engine's usage. The operation meters `prompt_tokens`, `completion_tokens`, `generations` and `ttft` (time to the first text chunk) beside its latency; the latency is the whole stream, so `[load] timeout` must allow a generation (seconds on a real engine, `120s` in the shipped scenario) |
