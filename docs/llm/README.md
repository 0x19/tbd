...
`think`; on llama-server the tier's `reasoning_on` / `reasoning_off`, the chat template's
own arguments, because the switch is the template's: gpt-oss takes `reasoning_effort` and
ignores `enable_thinking`, which told false made it reason longer, so both gpt-oss tiers
send effort `low` for false and `medium` for true, and a tier that says nothing sends
`enable_thinking`). gpt-oss cannot stop reasoning, only shorten it. Reasoning spends the token budget before any
