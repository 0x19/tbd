...
| `ui:www:check` | `ui-www` | `ui/www`: the lab render (`pnpm gen`: `docs/rfcs` and `docs/studies` into the site, refusing a public page that names a host, an address, a port, a path, a secret or a cluster name, as `file:line [rule] "text"`), prettier, eslint, tsc, the build, and the CV JSON in `crates/cv/assets` being current |
...
- `ghcr.io/<ORG>/tbd-runner`
- `ghcr.io/<ORG>/tbd-radar`
- `ghcr.io/<ORG>/tbd-arena`
- `ghcr.io/<ORG>/tbd-llm`
- `ghcr.io/<ORG>/tbd-cv`
