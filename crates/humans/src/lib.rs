//! The humans service: a gRPC server.
//!
...
use tbd_proto::humans::v1::humans_service_server::HumansServiceServer;
use tokio::net::TcpListener;
use tonic::transport::{Server, server::TcpIncoming};

pub use config::{Config, Overrides, Source};
pub use service::Humans;
pub use tbd_common::{
...
pub async fn serve_with(
    ...
    health_reporter
        .set_serving::<HumansServiceServer<Humans>>()
        .await;

    let reflection = tonic_reflection::server::Builder::configure()
        .register_encoded_file_descriptor_set(tbd_proto::humans::v1::DESCRIPTOR_SET)
        .register_encoded_file_descriptor_set(tonic_health::pb::FILE_DESCRIPTOR_SET)
        .build_v1()?;

    let service = Humans::new(config.ping.clone(), runtime);

    tracing::info!(%addr, version = tbd_common::VERSION, "humans listening");
    ...
    Server::builder()
        ...
        .add_service(HumansServiceServer::new(service))
        .serve_with_incoming_shutdown(incoming, shutdown)
        .await?;

    tracing::info!("humans stopped");
    Ok(())
}
