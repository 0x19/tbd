//! `humans` binary: load config, install telemetry, serve until signalled.
...
use tbd_humans::{Config, Overrides};

#[derive(Parser)]
#[command(name = "humans", version = tbd_common::VERSION, about = "The humans service")]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut telemetry = tbd_common::telemetry::init(&cli.telemetry, "humans")?;
    ...
    tbd_humans::serve(config, tbd_common::shutdown::signal()).await?;
