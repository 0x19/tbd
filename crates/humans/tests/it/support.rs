...
use tbd_humans::{Config, Runtime};
use tbd_proto::humans::v1::humans_service_client::HumansServiceClient;
...
pub async fn start_with(runtime: Runtime) -> Server {
    ...
    tokio::spawn(async move {
        tbd_humans::serve_with(listener, config, rt, async {
            let _ = stopped.await;
        })
        .await
        .unwrap();
...
impl Server {
    ...
    pub async fn client(&self) -> HumansServiceClient<Channel> {
        HumansServiceClient::new(self.channel().await)
    }
