<!-- tbd new service humans --kind grpc --port 50053 --metrics-port 9467 --bacon-key h (tbd-cli 0.1.0) -->
# crates/humans

The humans service. gRPC only. Scaffolded by `tbd new service` (docs/tbd/README.md);
...
- `service.rs`: the `HumansService` trait impl on `Humans`. Every RPC starts with
  `admit()`: count the request, start the `RequestTimer`, apply the fault handle, map
  a `Fault` to a gRPC status.
- `config.rs`: layered TOML, `configs/humans/base.toml` < `<env>.toml` < flags and
  `HUMANS_*` environment variables (`Overrides`). Every key lives in `base.toml`;
  `deny_unknown_fields` makes a mistyped key fail at start. `humans config` prints
...
- Observability: `tbd_common::telemetry::grpc_request_span` is the `trace_fn`, so every
  ...
  `[metrics] listen` (`HUMANS_METRICS_ADDR`), `None` for embedders.
...
shipped `configs/humans` and env `local`, and exposes the `Runtime` so tests can
