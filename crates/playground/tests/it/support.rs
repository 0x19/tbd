...
use tbd_playground::{Config, Runtime};
use tbd_proto::playground::v1::playground_service_client::PlaygroundServiceClient;
...
pub async fn start_with(runtime: Runtime) -> Server {
    ...
    tokio::spawn(async move {
        tbd_playground::serve_with(listener, config, rt, async {
            let _ = stopped.await;
        })
        .await
        .unwrap();
...
impl Server {
    ...
    pub async fn client(&self) -> PlaygroundServiceClient<Channel> {
        PlaygroundServiceClient::new(self.channel().await)
    }
