//! `playground` binary: load config, install telemetry, serve until signalled.
...
use tbd_playground::{Config, Overrides};

#[derive(Parser)]
#[command(name = "playground", version = tbd_common::VERSION, about = "The playground service")]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut telemetry = tbd_common::telemetry::init(&cli.telemetry, "playground")?;
    ...
    tbd_playground::serve(config, tbd_common::shutdown::signal()).await?;
