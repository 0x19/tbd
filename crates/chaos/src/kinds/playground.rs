//! The playground kind: an in-process `tbd-playground` with fault injection.
...
use tbd_proto::playground::v1::{PingRequest, playground_service_client::PlaygroundServiceClient};
...
pub static KIND: Kind = Kind {
    ...
    surface: "grpc",
    target: Some(Target {
        help: "Playground gRPC URL (through Envoy: the engine LB, matched by service name)",
        default_url: "http://127.0.0.1:50055",
    }),
    fields: &[],
    fault: true,
    store_fault: false,
    ...
    load_target: false,
    addable: true,
    parse: super::parse::<Playground>,
    checks: &[Check {
        name: "grpc_playground_ping",
        surface: "grpc",
        doc: "`Ping` echoes the message and is labelled a stub",
        run: |e| Box::pin(grpc_playground_ping(e)),
    }],
};

/// `[stack.playgrounds.<name>]` minus `listen`: a playground with an initial behaviour.
...
#[async_trait]
impl InstanceHandle for PlaygroundHandle {
    async fn ready(&self) -> bool {
        ...
        let req = HealthCheckRequest {
            service: "tbd.playground.v1.PlaygroundService".to_owned(),
        };
...
impl Service for Playground {
    ...
    async fn start(
        ...
        let config = Config {
            ...
            ping: Ping::default(),
            // `serve_with` starts no sandbox, so neither of these is read here;
            // they exist so the struct is complete.
            game: tbd_playground::Game::default(),
            sandbox: tbd_playground::SandboxConfig::default(),
        };
        let runtime = Runtime {
        ...
        let task = tokio::spawn(async move {
            if let Err(error) = tbd_playground::serve_with(listener, config, rt, async {
                let _ = stopped.await;
            })
            .await
            {
                tracing::error!(instance = %instance_name, %error, "playground exited with error");
            }
        });
        Ok(Instance::new(
            name,
...
/// `PlaygroundService/Ping` echoes and is labelled a stub. Not a named health
...
async fn grpc_playground_ping(e: Ep) -> Result<String, String> {
    let mut c = PlaygroundServiceClient::new(e.grpc()?);
    let r = c
        .ping(PingRequest {
