//! The radar kind: an in-process `tbd-radar` with fault injection.
...
use tbd_proto::radar::v1::{PingRequest, radar_service_client::RadarServiceClient};
use tbd_radar::{
    Config, Runtime,
...
pub static KIND: Kind = Kind {
    ...
    surface: "grpc",
    target: Some(Target {
        help: "Radar gRPC URL (through Envoy: the engine LB, matched by service name)",
        default_url: "http://127.0.0.1:50059",
    }),
    fields: &[],
    fault: true,
    store_fault: false,
    ...
    load_target: false,
    addable: true,
    parse: super::parse::<Radar>,
    checks: &[Check {
        name: "grpc_radar_ping",
        surface: "grpc",
        doc: "`Ping` echoes the message and is labelled a stub",
        run: |e| Box::pin(grpc_radar_ping(e)),
    }],
};

/// `[stack.radars.<name>]` minus `listen`: a radar with an initial behaviour.
...
#[async_trait]
impl InstanceHandle for RadarHandle {
    async fn ready(&self) -> bool {
        ...
        let req = HealthCheckRequest {
            service: "tbd.radar.v1.RadarService".to_owned(),
        };
...
impl Service for Radar {
    ...
    async fn start(
        ...
        let task = tokio::spawn(async move {
            if let Err(error) = tbd_radar::serve_with(listener, config, rt, async {
                let _ = stopped.await;
            })
            .await
            {
                tracing::error!(instance = %instance_name, %error, "radar exited with error");
            }
        });
        Ok(Instance::new(
            name,
...
/// `RadarService/Ping` echoes and is labelled a stub. Not a named health
...
async fn grpc_radar_ping(e: Ep) -> Result<String, String> {
    let mut c = RadarServiceClient::new(e.grpc()?);
    let r = c
        .ping(PingRequest {
