//! The arena kind: an in-process `tbd-arena` with fault injection.
...
pub static KIND: Kind = Kind {
    ...
    surface: "grpc",
    target: Some(Target {
        help: "Arena gRPC URL (through Envoy: the engine LB, matched by service name)",
        default_url: "http://127.0.0.1:50058",
    }),
    fields: &[],
    fault: true,
    store_fault: false,
...
#[async_trait]
impl InstanceHandle for ArenaHandle {
    async fn ready(&self) -> bool {
        ...
        let req = HealthCheckRequest {
            service: "tbd.arena.v1.ArenaService".to_owned(),
        };
...
impl Service for Arena {
    ...
    async fn start(
        ...
        let task = tokio::spawn(async move {
            if let Err(error) = tbd_arena::serve_with(listener, config, rt, async {
                let _ = stopped.await;
            })
            .await
            {
                tracing::error!(instance = %instance_name, %error, "arena exited with error");
            }
        });
        Ok(Instance::new(
            name,
...
/// `ArenaService/Ping` echoes and is labelled a stub. Not a named health
...
async fn grpc_arena_ping(e: Ep) -> Result<String, String> {
