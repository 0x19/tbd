//! What a kind *is*: the shape every service registration takes.
//!
//! The type and its parts, with no registry. Which kinds exist is a question
//! for whoever assembles them — `tbd-chaos` keeps that list, because it is the
//! one that knows about every service including the playground. Keeping the
//! type here lets anything hold a `&'static Kind` without depending on the
//! tool.
...
/// Running instances of every `load_target` kind in `all`, as load targets.
///
/// The addresses come from the stack, never from a caller: this is the only
/// way a load run learns where to aim.
#[must_use]
pub fn load_targets(stack: &Stack, all: &[&'static Kind]) -> Vec<load::Target> {
    stack
        .instances()
        .filter(|(_, i)| {
            all.iter()
                .find(|k| k.name == i.kind)
                .is_some_and(|k| k.load_target)
        })
...
#[must_use]
pub fn describe(all: &[&'static Kind]) -> Vec<KindInfo> {
    all.iter()
        .map(|k| KindInfo {
            name: k.name,
            label: k.label,
...
#[must_use]
pub fn markdown(all: &[&'static Kind]) -> String {
