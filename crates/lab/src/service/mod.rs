...
impl TaskHandle {
    /// Hold a running task and the channel that asks it to stop.
    #[must_use]
    pub fn new(stop: tokio::sync::oneshot::Sender<()>, task: tokio::task::JoinHandle<()>) -> Self {
        Self {
            stop: Some(stop),
            task: Some(task),
        }
    ...
    /// Ask the task to stop, then wait for it, giving up after five seconds.
