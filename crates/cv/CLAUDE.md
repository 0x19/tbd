...
The cv service. gRPC only. Scaffolded by `tbd new service` (docs/tbd/README.md);
...
- `service.rs`: the `CvService` trait impl on `Cv`. Every RPC starts with
  `admit()`: count the request, start the `RequestTimer`, apply the fault handle, map
  a `Fault` to a gRPC status.
- `config.rs`: layered TOML, `configs/cv/base.toml` < `<env>.toml` < flags and
  `CV_*` environment variables (`Overrides`). Every key lives in `base.toml`;
  `deny_unknown_fields` makes a mistyped key fail at start. `cv config` prints
...
- Observability: `tbd_common::telemetry::grpc_request_span` is the `trace_fn`, so every
  ...
  `[metrics] listen` (`CV_METRICS_ADDR`), `None` for embedders.
...
shipped `configs/cv` and env `local`, and exposes the `Runtime` so tests can
