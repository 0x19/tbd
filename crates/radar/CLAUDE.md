...
The radar service. gRPC only. Scaffolded by `tbd new service` (docs/tbd/README.md);
...
- `service.rs`: the `RadarService` trait impl on `Radar`. Every RPC starts with
...
- `config.rs`: layered TOML, `configs/radar/base.toml` < `<env>.toml` < flags and
  `RADAR_*` environment variables (`Overrides`). Every key lives in `base.toml`;
  `deny_unknown_fields` makes a mistyped key fail at start. `radar config` prints
...
- Observability: `tbd_common::telemetry::grpc_request_span` is the `trace_fn`, so every
  ...
  `[metrics] listen` (`RADAR_METRICS_ADDR`), `None` for embedders.
...
shipped `configs/radar` and env `local`, and exposes the `Runtime` so tests can
