//! `radar` binary: load config, install telemetry, serve until signalled.
...
use tbd_radar::{Config, Overrides};

#[derive(Parser)]
#[command(name = "radar", version = tbd_common::VERSION, about = "The radar service")]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut telemetry = tbd_common::telemetry::init(&cli.telemetry, "radar")?;
    ...
    tbd_radar::serve(config, tbd_common::shutdown::signal()).await?;
