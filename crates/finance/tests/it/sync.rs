...
#[tokio::test]
async fn an_attended_refresh_carries_the_address_but_is_counted_like_any_other() {
    ...
    // Three scheduled fetches spend the scheduler's share.
    for hour in 8..11 {
        syncer.tick(at(hour)).await.unwrap();
    }
    let calls = w.mock.calls();

    // The person asks: the call goes out with the address, from the reserve.
    let r = syncer
        .refresh(w.account, at(11), Some(psu.clone()))
        .await
        .unwrap();
    ...
    assert_eq!(
        used, 4,
        "the bank counts an attended fetch, so the budget does too"
    );
    assert_eq!(trigger, "attended");

    // The reserve is spent: asking again is refused without a call. Erste
    // measured a 429 on the fifth call whether or not the address went.
    assert_eq!(
        syncer
    ...
    assert_eq!(w.mock.calls(), calls + 2, "no call under a spent budget");

    // A backoff holds for the person as well: a 429 here would only extend it.
    sqlx::query(
        "update finance.accounts set sync_budget_used = 0, sync_backoff_until = $2 where id = $1",
    )
    .bind(w.account)
    .bind(at(20))
    .execute(&w.pool)
    ...
    assert_eq!(
        syncer
            .refresh(w.account, at(13), Some(psu.clone()))
            .await
            .unwrap(),
        Err(Skipped::BackingOff)
    );
    assert_eq!(w.mock.calls(), calls + 2, "no call under a backoff");
    let r = syncer.refresh(w.account, at(21), Some(psu)).await.unwrap();
    assert!(matches!(r, Ok(Outcome::Ok { .. })), "{r:?}");
}
