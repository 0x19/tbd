//! `runner` binary: load config, install telemetry, serve until signalled.
...
use tbd_runner::{Config, Overrides};

#[derive(Parser)]
#[command(name = "runner", version = tbd_common::VERSION, about = "The runner service")]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut telemetry = tbd_common::telemetry::init(&cli.telemetry, "runner")?;
    ...
    tbd_runner::serve(config, tbd_common::shutdown::signal()).await?;
