...
The llm service. gRPC only. Scaffolded by `tbd new service` (docs/tbd/README.md);
...
- `service.rs`: the `LlmService` trait impl on `Llm`. Every RPC starts with
  `admit()`: count the request, start the `RequestTimer`, apply the fault handle, map
  a `Fault` to a gRPC status.
- `config.rs`: layered TOML, `configs/llm/base.toml` < `<env>.toml` < flags and
  `LLM_*` environment variables (`Overrides`). Every key lives in `base.toml`;
  `deny_unknown_fields` makes a mistyped key fail at start. `llm config` prints
...
- Observability: `tbd_common::telemetry::grpc_request_span` is the `trace_fn`, so every
  ...
  `[metrics] listen` (`LLM_METRICS_ADDR`), `None` for embedders.
...
shipped `configs/llm` and env `local`, and exposes the `Runtime` so tests can
