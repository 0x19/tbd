...
use tbd_llm::{Config, Overrides};

#[derive(Parser)]
#[command(name = "llm", version = tbd_common::VERSION, about = "The llm service")]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut telemetry = tbd_common::telemetry::init(&cli.telemetry, "llm")?;
    ...
    tbd_llm::serve(config, tbd_common::shutdown::signal()).await?;
