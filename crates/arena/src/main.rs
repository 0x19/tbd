//! `arena` binary: load config, install telemetry, serve until signalled.
//! Stdout is the product of `config`, so the print lint is allowed here only.

#![allow(clippy::print_stdout)]

use clap::{Parser, Subcommand};
use tbd_arena::{Config, Overrides};
use tbd_common::telemetry::TelemetryArgs;

#[derive(Parser)]
#[command(name = "arena", version = tbd_common::VERSION, about = "The arena service")]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut telemetry = tbd_common::telemetry::init(&cli.telemetry, "arena")?;
    ...
    tbd_arena::serve(config, tbd_common::shutdown::signal()).await?;
