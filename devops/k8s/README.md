...
```
base/            engine, protocol, llm, cv, playground, finance, humans, ledger, envoy (deployments, headless services, configmap, envoy config)
...
|---|---|---|---|
| `local` | engine 2, protocol 2, envoy 2, llm 1, cv 1, playground 1, finance 1, humans 1, ledger 1 | `:dev`, `IfNotPresent`, imported by k3d | adds `envoy-lb.yaml` with host ports 18080 and 15051; `RUST_LOG=info,tbd=debug` |
| `dev` | engine 1, protocol 1, envoy 2, llm 1, cv 1, playground 1, finance 1, humans 1, ledger 1 | `:dev`, `Always` | `RUST_LOG=debug` |
