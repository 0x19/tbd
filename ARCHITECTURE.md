...
Two processes. The gateway owns every client-facing protocol and no business logic.
...
| `tbd-engine` | `tbd.engine.v1.Engine` implementation, health, reflection | common, proto |
| `tbd-protocol` | axum router: REST, SSE, WebSocket bridge, GraphQL, gateway gRPC | common, proto |
...
2. **The gateway has no business logic.** If a handler does more than translate and
...
5. **One port per service.** The gateway multiplexes HTTP/1.1 and h2c on 8080;
   the engine serves gRPC on 50051.
6. **Health is the orchestrator's job.** Images are distroless with no shell; probes are
   gRPC health on the engine, `/healthz` and `/readyz` on the gateway.
...
- **Errors**: `thiserror` enums in libraries, `anyhow` only in `main`. The gateway maps
  `tonic::Code` to HTTP status in `crates/protocol/src/error.rs`.
- **Tests**: one integration binary per service at `tests/it/`. The gateway's tests boot a
